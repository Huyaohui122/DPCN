...
for epoch in range(150):
    ...
    if epoch % 1== 0:
        ckpt_name = os.path.join(weights_dir, 'GAT_{0}.pth'.format(epoch))
        torch.save(model.state_dict(), ckpt_name)
print(strftime("%Y-%m-%d %H:%M:%S", localtime()))
np.savetxt(r'F:\Users\GAT\_acc.txt', epoch_acc,fmt='%.4f')
np.savetxt(r'F:\Users\GAT\precision.txt', epoch_precision,fmt='%.4f')
np.savetxt(r'F:\Users\GAT\loss.txt',epoch_losses,fmt='%.4f')
np.savetxt(r'F:\Users\GAT\_recall.txt', epoch_recall,fmt='%.4f')
np.savetxt(r'F:\Users\GAT\ma_f1.txt', epoch_ma_f1,fmt='%.4f')
