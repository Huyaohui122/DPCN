...
class Classifier(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes):
        ...
        self.sageconv1 = SAGEConv(in_dim, hidden_dim, "lstm")
        self.sageconv2 = SAGEConv(hidden_dim, 10, "mean")
    ...
    def forward(self, g):
        ...
        h = g.ndata['x'].float()
        h = F.leaky_relu(self.sageconv1(g, h))
        h = F.leaky_relu(self.sageconv2(g, h))
        g.ndata['x'] = h
...
for epoch in range(150):
    ...
    if epoch % 1== 0:
        ckpt_name = os.path.join(weights_dir, 'SAGE-2point_{0}.pth'.format(epoch))
        torch.save(model.state_dict(), ckpt_name)
print(strftime("%Y-%m-%d %H:%M:%S", localtime()))
np.savetxt(r'F:\Users\SAGE-2point\loss.txt',epoch_losses,fmt='%.4f')
np.savetxt(r'F:\Users\SAGE-2point\_acc.txt', epoch_acc,fmt='%.4f')
np.savetxt(r'F:\Users\SAGE-2point\precision.txt', epoch_precision,fmt='%.4f')
np.savetxt(r'F:\Users\SAGE-2point\_recall.txt', epoch_recall,fmt='%.4f')
np.savetxt(r'F:\Users\SAGE-2point\ma_f1.txt', epoch_ma_f1,fmt='%.4f')
