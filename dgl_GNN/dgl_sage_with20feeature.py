...
class Classifier(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes):
        ...
        self.sageconv2 = SAGEConv(hidden_dim, 256, "mean")
...
for epoch in range(150):
    ...
    if epoch % 1== 0:
        ckpt_name = os.path.join(weights_dir, 'SAGE_{0}.pth'.format(epoch))
        torch.save(model.state_dict(), ckpt_name)
print(strftime("%Y-%m-%d %H:%M:%S", localtime()))

np.savetxt(r'F:\Users\SAGE\loss.txt',epoch_losses,fmt='%.4f')
np.savetxt(r'F:\Users\SAGE\_acc.txt', epoch_acc,fmt='%.4f')
np.savetxt(r'F:\Users\SAGE\precision.txt', epoch_precision,fmt='%.4f')
np.savetxt(r'F:\Users\SAGE\_recall.txt', epoch_recall,fmt='%.4f')
np.savetxt(r'F:\Users\SAGE\ma_f1.txt', epoch_ma_f1,fmt='%.4f')
