...
class Classifier(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes):
        ...
        self.Gatconv1 = GATConv(in_dim,hidden_dim, 5)
        self.Gatconv2 = GATConv(hidden_dim*5, 256, 3)
        self.classify = nn.Linear(256, n_classes)

    def forward(self, g):
        h = g.ndata['x'].float()
        h = F.leaky_relu(self.Gatconv1(g, h))
        h = torch.flatten(h,1)
        h = F.leaky_relu(self.Gatconv2(g, h))
        g.ndata['x'] = h
...
for epoch in range(150):
    ...
    with torch.no_grad():
        for it, (batchg, label) in enumerate(test_loader):
            pred = torch.softmax(model(batchg)[: , -1], 1)
    ...
    if epoch % 1== 0:
        ckpt_name = os.path.join(weights_dir, 'dgl_GAT_2point_{0}.pth'.format(epoch))
        torch.save(model.state_dict(), ckpt_name)

print(strftime("%Y-%m-%d %H:%M:%S", localtime()))


np.savetxt(r'F:\Users\GAT-2point\loss.txt',epoch_losses,fmt='%.4f')
np.savetxt(r'F:\Users\GAT-2point\acc.txt', epoch_acc,fmt='%.4f')
np.savetxt(r'F:\Users\GAT-2point\precision.txt', epoch_precision,fmt='%.4f')
np.savetxt(r'F:\Users\GAT-2point\recall.txt', epoch_recall,fmt='%.4f')
np.savetxt(r'F:\Users\GAT-2point\ma_f1.txt', epoch_ma_f1,fmt='%.4f')
